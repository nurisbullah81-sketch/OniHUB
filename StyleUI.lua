...
if not _G.Cat then _G.Cat = { Player = game:GetService("Players").LocalPlayer, Settings = LoadSettings(), Labels = {} }
else _G.Cat.Player = game:GetService("Players").LocalPlayer; if not _G.Cat.Labels then _G.Cat.Labels = {} end; _G.Cat.Settings = LoadSettings() end

-- ==========================================
-- 2. WEBHOOK ENGINE (PROXY HYRA + DYNAMIC RARITY)
-- ==========================================
local Webhook = {}
...
function Webhook:Test(webhookURL)
    ...
    -- RAHASIA BYPASS: Pakai Hyra Proxy
...
local FloatCont = Instance.new("Frame", Gui); FloatCont.Size = UDim2.new(0, 70, 0, 40); FloatCont.Position = UDim2.new(0, 20, 0.5, -20); FloatCont.BackgroundTransparency = 1; FloatCont.ZIndex = 99999 
local FloatBtn = Instance.new("TextButton", FloatCont); FloatBtn.Size = UDim2.new(0, 40, 1, 0); FloatBtn.Position = UDim2.new(0, 30, 0, 0); FloatBtn.BackgroundColor3 = Theme.CardBG; FloatBtn.Text = "Cat"; FloatBtn.TextColor3 = Theme.CatPurple; FloatBtn.Font = Enum.Font.GothamBold; FloatBtn.TextSize = 16; FloatBtn.BorderSizePixel = 0; FloatBtn.AutoButtonColor = false; Instance.new("UICorner", FloatBtn).CornerRadius = UDim.new(0, 8); Instance.new("UIStroke", FloatBtn).Color = Theme.Line
local FloatDrag = Instance.new("TextButton", FloatCont); FloatDrag.Size = UDim2.new(0, 30, 1, 0); FloatDrag.BackgroundTransparency = 1; FloatDrag.Text = ""

local draggingFloat, dragStartFloat, startPosFloat
FloatDrag.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then draggingFloat = true; dragStartFloat = input.Position; startPosFloat = FloatCont.Position end end)
...
UserInput.InputChanged:Connect(function(input) if draggingFloat and input.UserInputType == Enum.UserInputType.MouseMovement then local delta = input.Position - dragStartFloat; FloatCont.Position = UDim2.new(startPosFloat.X.Scale, startPosFloat.X.Offset + delta.X, startPosFloat.Y.Scale, startPosFloat.Y.Offset + delta.Y) end end)

local Main = Instance.new("Frame", Gui); Main.Size = UDim2.new(0, 550, 0, 340); Main.Position = UDim2.new(0.5, -275, 0.5, -170); Main.BackgroundColor3 = Theme.MainBG; Main.BorderSizePixel = 0; Main.ClipsDescendants = true; Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", Main).Color = Theme.Line
FloatBtn.MouseButton1Click:Connect(function() Main.Visible = not Main.Visible end)

local Top = Instance.new("Frame", Main); Top.Size = UDim2.new(1, 0, 0, 35); Top.BackgroundColor3 = Theme.TopBG; Top.BorderSizePixel = 0; Instance.new("UICorner", Top).CornerRadius = UDim.new(0, 6)
...
local SearchFrame = Instance.new("Frame", Side); SearchFrame.Size = UDim2.new(1, -16, 0, 30); SearchFrame.Position = UDim2.new(0, 8, 0, 10); SearchFrame.BackgroundColor3 = Theme.CardBG; SearchFrame.BorderSizePixel = 0; Instance.new("UICorner", SearchFrame).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", SearchFrame).Color = Theme.Line
...
local Pages, AllToggles = {}, {}

local function CreateTab(name, isFirst)
    local Btn = Instance.new("TextButton", SideScroll); Btn.Size = UDim2.new(1, 0, 0, 32); Btn.BackgroundColor3 = isFirst and Theme.TabOn or Theme.TabOff; Btn.Text = "    " .. name; Btn.TextColor3 = isFirst and Theme.Text or Theme.TextDim; Btn.Font = Enum.Font.GothamMedium; Btn.TextSize = 12; Btn.BorderSizePixel = 0; Btn.TextXAlignment = Enum.TextXAlignment.Left; Btn.AutoButtonColor = false; Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 6)
    ...
    local Indicator = Instance.new("Frame", Btn); Indicator.Size = UDim2.new(0, 3, 0, 14); Indicator.Position = UDim2.new(0, 4, 0.5, -7); Indicator.BackgroundColor3 = Theme.Accent; Indicator.BorderSizePixel = 0; Instance.new("UICorner", Indicator).CornerRadius = UDim.new(1, 0); Indicator.Visible = isFirst
    local Page = Instance.new("ScrollingFrame", ContentArea); Page.Size = UDim2.new(1, -16, 1, -16); Page.Position = UDim2.new(0, 8, 0, 8); Page.BackgroundColor3 = Theme.PageBG; Page.BackgroundTransparency = 0; Page.ScrollBarThickness = 2; Page.ScrollBarImageColor3 = Theme.TextDim; Page.Visible = isFirst; Page.BorderSizePixel = 0; Instance.new("UICorner", Page).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", Page).Color = Theme.Line
    ...
    Btn.MouseButton1Click:Connect(function() for tName, data in pairs(Pages) do local active = (tName == name); data.Page.Visible = active; data.Ind.Visible = active; TweenService:Create(data.Btn, TweenInfo.new(0.15), {BackgroundColor3 = active and Theme.TabOn or Theme.TabOff, TextColor3 = active and Theme.Text or Theme.TextDim}):Play(); TweenService:Create(data.Stroke, TweenInfo.new(0.15), {Transparency = active and 0 or 0.3}):Play() end end)
    return Page
end
...
local function CreateToggle(parent, text, description, stateRef, callback)
    local frameHeight = description and 52 or 36; local F = Instance.new("TextButton", parent); F.LayoutOrder = #parent:GetChildren(); F.Size = UDim2.new(1, 0, 0, frameHeight); F.BackgroundColor3 = Theme.CardBG; F.BorderSizePixel = 0; F.Text = ""; F.AutoButtonColor = false; Instance.new("UICorner", F).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", F).Color = Theme.Line
...
local function CreateLabel(parent, text, description)
    local frameHeight = description and 45 or 30; local F = Instance.new("Frame", parent); F.LayoutOrder = #parent:GetChildren(); F.Size = UDim2.new(1, 0, 0, frameHeight); F.BackgroundColor3 = Theme.CardBG; F.BorderSizePixel = 0; Instance.new("UICorner", F).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", F).Color = Theme.Line
...
SearchBox:GetPropertyChangedSignal("Text"):Connect(function() local query = string.lower(SearchBox.Text); for _, toggle in ipairs(AllToggles) do local text = string.lower(toggle.Label.Text); toggle.Btn.Visible = query == "" or string.find(text, query) ~= nil end end)

-- ==========================================
-- BUILD TABS & KONTEN UI LU
-- ==========================================
...
CreateToggle(AutoFarmTab, "Auto Attack", "Automatically swing weapon", _G.Cat.Settings.AutoAttack, function(state) _G.Cat.Settings.AutoAttack = state end)

-- DEVIL FRUITS (Kategori 1)
CreateSection(DevilFruitsTab, "FRUIT FINDER")
CreateToggle(DevilFruitsTab, "Fruit ESP", "Show text on spawned fruits", _G.Cat.Settings.FruitESP, function(state) _G.Cat.Settings.FruitESP = state end)
...
CreateToggle(DevilFruitsTab, "TP Fruits", "Instant teleport to fruits", _G.Cat.Settings.InstantTPFruit, function(state) _G.Cat.Settings.InstantTPFruit = state end)
CreateToggle(DevilFruitsTab, "Auto Store Fruits", "Store collected fruits", _G.Cat.Settings.AutoStoreFruit, function(state) _G.Cat.Settings.AutoStoreFruit = state end)
CreateToggle(DevilFruitsTab, "Auto Hop Server", "Hop if no fruits", _G.Cat.Settings.AutoHop, function(state) _G.Cat.Settings.AutoHop = state end)

-- DEVIL FRUITS (Kategori 2)
CreateSection(DevilFruitsTab, "DISCORD WEBHOOK")
CreateToggle(DevilFruitsTab, "Fruit Webhook", "Send alerts to Discord", _G.Cat.Settings.FruitWebhook, function(state) _G.Cat.Settings.FruitWebhook = state end)

local WHConfig = Instance.new("Frame", DevilFruitsTab); WHConfig.LayoutOrder = #DevilFruitsTab:GetChildren(); WHConfig.Size = UDim2.new(1, 0, 0, 106); WHConfig.BackgroundTransparency = 1
...
local WHURLFrame = Instance.new("Frame", WHConfig); WHURLFrame.LayoutOrder = 1; WHURLFrame.Size = UDim2.new(1, 0, 0, 32); WHURLFrame.BackgroundColor3 = Theme.CardBG; WHURLFrame.BorderSizePixel = 0; Instance.new("UICorner", WHURLFrame).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", WHURLFrame).Color = Theme.Line
local WHURLBox = Instance.new("TextBox", WHURLFrame); WHURLBox.Size = UDim2.new(1, -16, 1, 0); WHURLBox.Position = UDim2.new(0, 8, 0, 0); WHURLBox.BackgroundTransparency = 1; WHURLBox.Text = _G.Cat.Settings.FruitWebhookURL ~= "" and _G.Cat.Settings.FruitWebhookURL or ""; WHURLBox.TextColor3 = Theme.Text; WHURLBox.PlaceholderText = "Paste Discord Webhook URL here..."; WHURLBox.PlaceholderColor3 = Theme.TextDim; WHURLBox.Font = Enum.Font.GothamMedium; WHURLBox.TextSize = 11; WHURLBox.TextXAlignment = Enum.TextXAlignment.Left; WHURLBox.ClearTextOnFocus = false
WHURLBox.FocusLost:Connect(function() _G.Cat.Settings.FruitWebhookURL = WHURLBox.Text; SaveSettings() end)

local WHRarityBtn = Instance.new("TextButton", WHConfig); WHRarityBtn.LayoutOrder = 2; WHRarityBtn.Size = UDim2.new(1, 0, 0, 28); WHRarityBtn.BackgroundColor3 = Theme.SideBG; WHRarityBtn.BorderSizePixel = 0; WHRarityBtn.Text = "Rarity: " .. _G.Cat.Settings.FruitWebhookRarity; WHRarityBtn.TextColor3 = Theme.Text; WHRarityBtn.Font = Enum.Font.GothamMedium; WHRarityBtn.TextSize = 11; WHRarityBtn.AutoButtonColor = false; Instance.new("UICorner", WHRarityBtn).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", WHRarityBtn).Color = Theme.Line
local rarityOptions = {"All Fruits", "Legendary & Mythical", "Mythical Only"}
WHRarityBtn.MouseButton1Click:Connect(function() local current = _G.Cat.Settings.FruitWebhookRarity; local nextIndex = 1; for i, v in ipairs(rarityOptions) do if v == current then nextIndex = (i % #rarityOptions) + 1 break end end; _G.Cat.Settings.FruitWebhookRarity = rarityOptions[nextIndex]; WHRarityBtn.Text = "Rarity: " .. rarityOptions[nextIndex]; SaveSettings() end)

local WHTestBtn = Instance.new("TextButton", WHConfig); WHTestBtn.LayoutOrder = 3; WHTestBtn.Size = UDim2.new(1, 0, 0, 28); WHTestBtn.BackgroundColor3 = Theme.SideBG; WHTestBtn.BorderSizePixel = 0; WHTestBtn.Text = "Test Webhook"; WHTestBtn.TextColor3 = Theme.CatPurple; WHTestBtn.Font = Enum.Font.GothamBold; WHTestBtn.TextSize = 11; WHTestBtn.AutoButtonColor = false; Instance.new("UICorner", WHTestBtn).CornerRadius = UDim.new(0, 6); Instance.new("UIStroke", WHTestBtn).Color = Theme.Line
WHTestBtn.MouseButton1Click:Connect(function()
    WHTestBtn.Text = "Sending..."
    -- PANGGIL LANGSUNG FUNGSI WEBHOOK DARI ATAS
    local ok, err = Webhook:Test(_G.Cat.Settings.FruitWebhookURL)
