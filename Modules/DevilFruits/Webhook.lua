...
function Webhook:Test(webhookURL)
    ...
    local payload = HttpService:JSONEncode({
        ...
        embeds  = {{
            ...
            description = "Script connected via Proxy.",
            color       = 32768,
...
WHTestBtn.MouseButton1Click:Connect(function()
    ...
    if ok then
        ...
        -- Potong pesan error biar ga panjang
        local shortErr = string.sub(tostring(err), 1, 15)
        WHTestBtn.Text = "Fail: " .. shortErr
    end
...
-- [[ ==========================================
--      4. FRUIT DETECTION ENGINE
--    ========================================== ]]
task.spawn(function()
    local player = Players.LocalPlayer
        or Players.PlayerAdded:Wait()

    -- Tunggu sampai player instance valid
    while not player do
        task.wait(0.5)
        player = Players.LocalPlayer
    end

    -- // State: Anti-Spam Memory
    local fruitCooldowns = {}

    -- // Function: Core Identification Logic
    local function CheckForFruit(item)
        if not item:IsA("Tool") then return end

        -- Gate: Cek toggle aktif atau ga
        if not Settings.FruitWebhook then return end

        -- // VALIDATION 1: Handle Check
        -- Bedain antara Buah Fisik (ada Handle) vs Ability/Kekuatan (ga ada)
        task.wait(0.1) -- Jeda biar instance ke-load penuh
        if not item:FindFirstChild("Handle") then
            return -- Skip kalo cuma ability
        end

        local rawName   = item.Name
        local lowerName = string.lower(rawName)

        -- // VALIDATION 2: Pattern Check
        ...
        elseif hasFruitWord then
            -- Format: "Ice Fruit" -> "Ice"
            cleanFruitName = string.gsub(rawName, "(%s?)[Ff][Rr][Uu][Ii][Tt]", "")
        end

        -- // VALIDATION 4: Anti-Spam Cooldown
        -- Mencegah notif berulang untuk buah yang sama dalam waktu singkat
        if fruitCooldowns[cleanFruitName] then
            return
        end
    ...
    -- // Event Listeners: Backpack & Character
    local function ConnectListeners(char)
        -- Monitor Item Equipped
    ...
    -- Initial Scan for Current Session
