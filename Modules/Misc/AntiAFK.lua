-- [[ ==========================================
--      MODULE: MISC - ANTI-AFK & AUTO-TEAM
--    ========================================== ]]

local VirtualUser       = game:GetService("VirtualUser")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Players           = game:GetService("Players")

local Me = Players.LocalPlayer

-- // Framework Initialization
repeat
    task.wait(0.1)
until _G.Cat and _G.Cat.UI and _G.Cat.Settings

local UI         = _G.Cat.UI
local Settings   = _G.Cat.Settings
local SafeInvoke = _G.Cat.SafeInvoke

-- ==========================================
-- 1. UI INITIALIZATION
-- ==========================================
local Page = UI.CreateTab("Misc", false)
UI.CreateSection(Page, "AUTOMATION & PROTECTION")

-- // Toggle: Anti-AFK
if type(Settings.AntiAFK) ~= "boolean" then
    Settings.AntiAFK = true
end

UI.CreateToggle(
    Page,
    "Anti AFK",
    "Prevents 20-minute idle kick",
    Settings.AntiAFK,
    function(state)
        Settings.AntiAFK = state
    end
)

-- // Toggle: Auto Team
if type(Settings.AutoTeam) ~= "boolean" then
    Settings.AutoTeam = false
end

UI.CreateToggle(
    Page,
    "Auto Team Marines",
    "Automatically picks Marines on join",
    Settings.AutoTeam,
    function(state)
        Settings.AutoTeam = state
    end
)

-- ==========================================
-- 2. LOGIC: ANTI-IDLE SYSTEM
-- ==========================================
Me.Idled:Connect(function()
    if Settings.AntiAFK then
        pcall(function()
            VirtualUser:CaptureController()
            VirtualUser:ClickButton2(Vector2.new(0, 0))
        end)
    end
end)

-- ==========================================
-- 3. LOGIC: SMART AUTO TEAM
-- ==========================================
task.spawn(function()
    while task.wait(1.5) do
        -- Hanya jalan kalo fitur aktif dan belum punya tim
        if Settings.AutoTeam and Me.Team == nil then
            pcall(function()
                -- 1. Langsung hit Remote (Optimized: No UI Scanning)
                local remotes = ReplicatedStorage:FindFirstChild("Remotes")
                local commF   = remotes and remotes:FindFirstChild("CommF_")

                if commF then
                    SafeInvoke(commF, "SetTeam", "Marines")
                end

                task.wait(1)

                -- 2. Cleanup GUI jika berhasil join
                if Me.Team ~= nil then
                    local playerGui = Me:FindFirstChild("PlayerGui")
                    if playerGui then
                        -- Spesifik cari GUI, biar ga berat
                        local chooseTeam = playerGui:FindFirstChild("ChooseTeam")
                        if chooseTeam then
                            chooseTeam.Enabled = false
                        end
                    end

                    -- 3. Reset Camera (Fix Camera Bug)
                    local camera = workspace.CurrentCamera
                    camera.CameraType = Enum.CameraType.Custom

                    -- FIX: Validasi Humanoid dulu biar ga error
                    if Me.Character then
                        local hum = Me.Character:FindFirstChild("Humanoid")
                        if hum then
                            camera.CameraSubject = hum
                        end
                    end
                end
            end)
        end
    end
end)