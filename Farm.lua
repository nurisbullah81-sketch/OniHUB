-- CatHUB SUPREMACY: Farm Module v12.0
local UI = _G.UI_Lib
local LP = game:GetService("Players").LocalPlayer
local VIM = game:GetService("VirtualInputManager")

local Tab = UI:NewTab("Main Farm")
UI:NewSelector(Tab, "WeaponType", "Current Weapon", {"Melee", "Sword", "Fruit"})
UI:NewSwitch(Tab, "AutoFarm", "Auto Farm NPC")
UI:NewSwitch(Tab, "AutoAttack", "Aggressive Smooth Attack")
...
local function Equip()
    ...
    for _,v in pairs(LP.Backpack:GetChildren()) do
        if (type == "Melee" and v.ToolTip == "Melee") or (type == "Sword" and v.ToolTip == "Sword") or (type == "Fruit" and v.ToolTip == "Blox Fruit") then
            LP.Character.Humanoid:EquipTool(v)
        end
    end
end

-- ULTIMATE FAST ATTACK
task.spawn(function()
    while task.wait() do
...
task.spawn(function()
    while task.wait() do
        if UI.Settings.AutoFarm then
            ...
            if target then
                -- Terbang Tinggi biar aman dari hit NPC (Y + 40)
                LP.Character.HumanoidRootPart.CFrame = target.PrimaryPart.CFrame * CFrame.new(0, 40, 0) * CFrame.Angles(math.rad(-90), 0, 0)
