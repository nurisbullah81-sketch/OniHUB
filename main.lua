-- CatHUB FREEMIUM: Logic Core (Noclip & Instant Stop)
...
local function AutoStore()
    ...
    if char then
        ...
        if tool and tool.Name:lower():find("fruit") then
            -- Remote standard Blox Fruits untuk menyimpan buah
...
local function TweenTo(target)
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        currentTween = TweenService:Create(char.HumanoidRootPart, tInfo, {CFrame = targetCF})
...
-- Monitor Toggles for Instant Stop
task.spawn(function()
    while task.wait(0.1) do
        if not UI_Module.Settings.Tween_Enabled and currentTween then
            currentTween:Cancel()
            currentTween = nil
        end
