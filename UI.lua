-- CatHUB SUPREMACY: UI Module v11.0
local UserInputService = game:GetService("UserInputService")
local CoreGui = game:GetService("CoreGui")
local TweenService = game:GetService("TweenService")
local HttpService = game:GetService("HttpService")

if CoreGui:FindFirstChild("CatHUB_v11") then CoreGui.CatHUB_v11:Destroy() end
...
local UI = {
    ...
    Settings = {
        ...
        WeaponType = "Melee", -- Default: Melee, Sword, Fruit
        -- FRUITS
        ESP_Fruits = false, AutoTweenFruit = false, AutoStore = false,
        -- PVP
        LockAim = false, AntiStun = true, ESP_Players = false,
        RunSpeed = 16, JumpPower = 50, TweenSpeed = 300
    }
}

function UI:Save() pcall(function() writefile("CatHUB_v11.json", HttpService:JSONEncode(self.Settings)) end) end
function UI:Load() 
    if isfile and isfile("CatHUB_v11.json") then 
        local s, d = pcall(function() return HttpService:JSONDecode(readfile("CatHUB_v11.json")) end)
        if s then for k,v in pairs(d) do self.Settings[k] = v end end
    end 
end
UI:Load()

local ScreenGui = Instance.new("ScreenGui", CoreGui); ScreenGui.Name = "CatHUB_v11"
local Main = Instance.new("Frame", ScreenGui)
Main.Size = UDim2.new(0, 540, 0, 360) -- Lebar & Slim
...
Title.Text = "CATHUB SUPREMACY v11.0"; Title.TextColor3 = Color3.fromRGB(255, 255, 255)
...
Sidebar.Name = "Sidebar"; Sidebar.Size = UDim2.new(0, 135, 1, -42); Sidebar.Position = UDim2.new(0, 5, 0, 37)
...
ContentArea.Name = "Content"; ContentArea.Size = UDim2.new(1, -150, 1, -42); ContentArea.Position = UDim2.new(0, 145, 0, 37)
...
function UI:NewSwitch(p, s, t, cb)
    ...
    local Sw = Instance.new("TextButton", F); Sw.Size = UDim2.new(0, 32, 0, 16); Sw.Position = UDim2.new(1, -42, 0.5, -8); Sw.BackgroundColor3 = self.Settings[s] and self.Accent or Color3.fromRGB(45, 45, 45); Sw.Text = ""; Instance.new("UICorner", Sw).CornerRadius = UDim.new(1, 0)
